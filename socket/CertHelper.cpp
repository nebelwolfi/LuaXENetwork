...
{
	if (DnsNameCompare_W(HostName.c_str(), pRequiredName)) // The HostName is the RequiredName
		return true;
...
bool MatchCertificateName(PCCERT_CONTEXT pCertContext, LPCWSTR pszRequiredName) {
    ...
    CertGetNameStringW(pCertContext, CERT_NAME_ATTR_TYPE, 0, const_cast<char*>(szOID_COMMON_NAME), &CommonName[0], dwCommonNameLength);
...
SECURITY_STATUS CertFindServerCertificateByName(PCCERT_CONTEXT & pCertContext, LPCWSTR pszSubjectName, bool fUserStore)
{
		...
		pCertContext))) // If this points to a valid certificate it will act as starting point and also be closed
	{
		...
		if (!CertGetNameStringW(pCertContext, CERT_NAME_FRIENDLY_DISPLAY_TYPE, 0, nullptr, pszFriendlyNameString, _countof(pszFriendlyNameString)))
		{
		...
		if (!CertGetNameStringW(pCertContext, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, nullptr, pszNameString, _countof(pszNameString)))
...
SECURITY_STATUS CertFindClientCertificate(PCCERT_CONTEXT & pCertContext, const LPCWSTR pszSubjectName, bool fUserStore)
{
	...
	{
		//ShowCertInfo(pCertContext);
		if (!CertGetNameStringW(pCertContextCurrent, CERT_NAME_FRIENDLY_DISPLAY_TYPE, 0, nullptr, pszFriendlyNameString, _countof(pszFriendlyNameString)))
		{
			DebugMsg("CertGetNameString failed getting friendly name.");
			continue;
		}
		DebugMsg("Certificate '%S' is allowed to be used for client authentication.", pszFriendlyNameString);
		if (!CertGetNameStringW(pCertContextCurrent, CERT_NAME_SIMPLE_DISPLAY_TYPE, 0, nullptr, pszNameString, _countof(pszNameString)))
		{
			DebugMsg("CertGetNameString failed getting subject name.");
			continue;
		}
...
SECURITY_STATUS CertFindServerCertificateUI(PCCERT_CONTEXT & pCertContext, LPCWSTR pszSubjectName, bool fUserStore)
{
	...
	{  // Not linked yet, find the function in the DLL
		HINSTANCE CryptUIDLL = LoadLibraryW(L"CryptUI.dll");
		if (CryptUIDLL)
...
std::wstring GetCertName(PCCERT_CONTEXT pCertContext)
{
	...
	certName.resize(128);
  auto good = CertGetNameStringW(pCertContext, CERT_NAME_FRIENDLY_DISPLAY_TYPE, 0, nullptr, &certName[0], static_cast<DWORD>(certName.capacity()));
	if (good)
	{
...
PCCERT_CONTEXT CreateCertificate(bool useMachineStore, LPCWSTR Subject, LPCWSTR FriendlyName, LPCWSTR Description, bool forClient)
{
	...
	if (CertStrToNameW(X509_ASN_ENCODING, X500.c_str(), CERT_X500_NAME_STR, nullptr, nullptr, &cbEncoded, nullptr))
		DebugMsg("Success");
	else
	{
		// Error
	...
	DebugMsg(("CertStrToName... "));
	if (CertStrToNameW(X509_ASN_ENCODING, X500.c_str(), CERT_X500_NAME_STR, nullptr, &CertName[0], &cbEncoded, nullptr))
		DebugMsg("Success");
	else
	{
		// Error
