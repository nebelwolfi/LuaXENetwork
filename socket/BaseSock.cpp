...
void CBaseSock::StartRecvTimerInternal()
{
	...
	RecvEndTime = std::chrono::duration_cast<std::chrono::seconds>(p1.time_since_epoch()).count() + RecvTimeoutSeconds;
}

void CBaseSock::StartSendTimerInternal()
{
	const auto p1 = std::chrono::system_clock::now();
	SendEndTime = std::chrono::duration_cast<std::chrono::seconds>(p1.time_since_epoch()).count() + SendTimeoutSeconds;
}
...
int CBaseSock::RecvPartial(LPVOID lpBuf, const size_t Len)
{
	...
	const auto p1 = std::chrono::system_clock::now();
	const auto SecondsLeft = RecvEndTime - std::chrono::duration_cast<std::chrono::seconds>(p1.time_since_epoch()).count();
	if (SecondsLeft <= 0)
	{
	...
	{
		const DWORD dwWait = WaitForMultipleObjects(2, hEvents, false, static_cast<DWORD>(SecondsLeft) * 1000); //SecondsLeft insteasd of 10
		switch (dwWait)
		{
...
bool CBaseSock::Connect(LPCWSTR HostName, USHORT PortNumber)
{
    ...
    l.l_onoff = 1;
    l.l_linger = 0;
    setsockopt(ActualSocket, SOL_SOCKET, SO_LINGER, (char*)&l, sizeof(linger));

	// Note that WSAConnectByName requires Vista or Server 2008
	bSuccess = WSAConnectByNameW(ActualSocket, const_cast<LPWSTR>(HostName),
	...
	if (!bSuccess) {
		LastError = WSAGetLastError();
		//printf("**** WsaConnectByName Error %d connecting to \"%S\" (%S)",
		//	LastError,
		//	HostName,
		//	PortName);
	...
	if (iResult == SOCKET_ERROR) {
		LastError = WSAGetLastError();
		//printf("setsockopt for SO_UPDATE_CONNECT_CONTEXT failed with error: %d", LastError);
		CloseAndInvalidateSocket();
		return false;
	}
