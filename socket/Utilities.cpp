...
std::wstring string_format(const WCHAR* pszFormat, ...) {
	int n = lstrlenW(pszFormat) * 2; /* Reserve two times as much as the length of the fmt_str */
	if (n < 10) n = 10;
...
std::wstring GetHostName(COMPUTER_NAME_FORMAT WhichName)
{
	...
	if (ERROR_SUCCESS == ::GetComputerNameEx(WhichName, nullptr, &NameLength))
	{
		...
		if (::GetComputerNameExW(WhichName, &ComputerName[0], &NameLength))
		{
			return ComputerName;
		}
	}
...
std::wstring GetCurrentUserName()
{
	...
	if (ERROR_SUCCESS == ::GetUserName(nullptr, &NameLength))
	{
		...
		if (::GetUserNameW(&UserName[0], &NameLength))
		{
			return UserName;
		}
	}
...
std::wstring WinErrorMsg(int nErrorCode)
{
	...
	try
	{
		auto len = FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM,
			nullptr, nErrorCode,
