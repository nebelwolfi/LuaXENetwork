...
PSecurityFunctionTableW CSSLClient::g_pSSPI = nullptr;
...
HRESULT CSSLClient::Initialize(LPCWSTR ServerName, const void * const lpBuf, const int Len)
{
	...
	hr = g_pSSPI->QueryContextAttributesW(m_hContext.getunsaferef(), SECPKG_ATTR_STREAM_SIZES, &Sizes);
...
{
	g_pSSPI = InitSecurityInterfaceW();
...
#pragma warning (suppress: 4238)
		...
		scRet == SEC_I_INCOMPLETE_CREDENTIALS)
	{
		...
		scRet = g_pSSPI->InitializeSecurityContextW(m_ClientCreds.getunsaferef(),
			...
			FAILED(scRet) && (dwSSPIOutFlags & ISC_RET_EXTENDED_ERROR))
		{
			...
			HRESULT hr = g_pSSPI->QueryContextAttributesW(m_hContext.getunsaferef(), SECPKG_ATTR_REMOTE_CERT_CONTEXT, hServerCertContext.set());
...
HRESULT CSSLClient::DisconnectSSL()
{
	...
	if (FAILED(Status))
	{
		DebugMsg("**** Error 0x%llX returned by ApplyControlToken", Status);
		return Status;
	}
	...
	Status = g_pSSPI->InitializeSecurityContextW(
	...
	if (FAILED(Status))
	{
		DebugMsg("**** Error 0x%llX returned by AcceptSecurityContext", Status);
		return Status;
	}
...
SECURITY_STATUS CSSLClient::GetNewClientCredentials()
{
	...
	Status = g_pSSPI->QueryContextAttributesW(m_hContext.getunsaferef(),
...
SECURITY_STATUS CSSLClient::CreateCredentialsFromCertificate(PCredHandle phCreds, PCCERT_CONTEXT pCertContext)
{
	...
	Status = g_pSSPI->AcquireCredentialsHandleW(
		...
		const_cast<WCHAR*>(UNISP_NAME_W), // Name of package
	...
	if (Status != SEC_E_OK)
	{
		...
		if (Status == SEC_E_UNKNOWN_CREDENTIALS)
			...
			DebugMsg("**** Error 0x%llX returned by AcquireCredentialsHandle. LastError=%d.", Status, dw);
		return Status;
	}
